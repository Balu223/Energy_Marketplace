...
import { UserResponseDto, UserService, CreateProfileDto,} from '../../../../Core/Services/user.service';
...
export class AdminPanelComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.items$.subscribe((items) => {
      const purchaseControls = items.map((i) => this.fb.control(i.purchase_Price_Per_Unit ?? 0));
      const saleControls = items.map((i) => this.fb.control(i.sale_Price_Per_Unit ?? 0));

      this.form.setControl('newPurchase', this.fb.array(purchaseControls));
      this.form.setControl('newSale', this.fb.array(saleControls));
    });
  }
  ...
  loadUsers() {
    this.userService
      .listUsers()
      .pipe(map((users) => [...users].sort((a, b) => a.user_Id - b.user_Id)))
  ...
  openUserCreate() {
    const dialogRef = this.dialog.open(CreateUserDialogComponent, {
      panelClass: 'create-user-dialog-panel',
    });
    dialogRef.afterClosed().subscribe((result: boolean | undefined) => {
      if (result) {
        this.loadUsers();
